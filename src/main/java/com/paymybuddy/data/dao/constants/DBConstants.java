...
public class DBConstants {
    ...
    public static final String ADD_UNIQUE_USER = "INSERT INTO users (FirstName, LastName, Address, City, Zip, Phone, Email, Password, Balance, BankAcctIBAN, BankAcctBIC)\n" +
            "SELECT * FROM (SELECT ? AS FirstName, ? AS LastName, ? AS Address, ? AS City, ? AS Zip, ? AS Phone, ? AS Email, ? AS Password, ? AS Balance, ? AS BankAcctIBAN, ? AS BankAcctBIC) AS temp\n" +
            "WHERE NOT EXISTS (\n" +
            "    SELECT Email FROM users WHERE Email = ?\n" +
            ") LIMIT 1;";
    public static final String UPDATE_USER_AUTHED = "UPDATE users SET FirstName=?, LastName=?, Address=?, City=?, Zip=?, Phone=?, Email=?, BankAcctIBAN=?, BankAcctBIC=? WHERE AcctID=?";
    ...
    public static final String ADD_UNIQUE_RELATIONSHIP_BY_EMAIL_WITH_ACTIVE = "INSERT INTO userrelationships (ListOwnerID, ListFriendID)\n" +
            "SELECT * FROM (SELECT ? AS ListOwnerID, AcctID FROM (SELECT AcctID FROM users WHERE Email = ? ) AS ListFriendID) AS temp\n" +
    ...
    public static final String GET_UNPROCESSED_BANK_TRANSACTIONS_DETAILS = "SELECT * FROM banktransactions WHERE Processed=false AND Cancelled=false";
